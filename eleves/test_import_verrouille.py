...
class ImportVerrouilleTests(TestCase):
    ...
    def test_erreur_allocation_annule_deverrouillage(self):
        ...
        from paiements.views import _valider_paiement_impl, ensure_echeancier_for_eleve
        paiement = self.paiement()
        echeancier = ensure_echeancier_for_eleve(self.eleve)
        echeancier.frais_inscription_du = 10000
        echeancier.save()
        with patch('paiements.views._allocate_payment_to_echeancier', side_effect=ValueError('Allocation impossible')):
