import sqlite3
import mysql.connector
from mysql.connector import Error
import sys

SQLITE_DB = 'db.sqlite3'
MYSQL_CONFIG = {
    'host': 'GSHadjaKanfingDiane.mysql.pythonanywhere-services.com',
    'user': 'GSHadjaKanfingDi',
    'password': 'FELIXSUZANELENO1994@',
    'database': 'GSHadjaKanfingDi$default',
    'port': 3306,
    'charset': 'utf8mb4',
    'use_unicode': True
}

def drop_all_tables(mysql_cursor, mysql_conn):
    """Supprime toutes les tables existantes"""
    try:
        print("🗑️  Suppression de toutes les tables existantes...")
        
        # Désactiver les contraintes
        mysql_cursor.execute("SET FOREIGN_KEY_CHECKS=0;")
        
        # Récupérer toutes les tables
        mysql_cursor.execute("SHOW TABLES;")
        tables = [row[0] for row in mysql_cursor.fetchall()]
        
        print(f"   {len(tables)} tables à supprimer...")
        
        # Supprimer chaque table
        for table in tables:
            mysql_cursor.execute(f"DROP TABLE IF EXISTS `{table}`;")
        
        mysql_conn.commit()
        mysql_cursor.execute("SET FOREIGN_KEY_CHECKS=1;")
        print("✅ Toutes les tables supprimées\n")
        return True
    except Error as e:
        print(f"❌ Erreur: {e}")
        return False

def get_sqlite_tables(cursor):
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name;")
    return [table[0] for table in cursor.fetchall()]

def get_table_schema(cursor, table_name):
    cursor.execute(f"PRAGMA table_info(`{table_name}`);")
    return cursor.fetchall()

def get_table_pk(cursor, table_name):
    cursor.execute(f"PRAGMA table_info(`{table_name}`);")
    cols = cursor.fetchall()
    return [col[1] for col in cols if col[5] > 0]

def sqlite_to_mysql_type(sqlite_type, col_name):
    sqlite_type = (sqlite_type or 'TEXT').upper()
    
    if 'CHAR' in sqlite_type or 'VARCHAR' in sqlite_type:
        return sqlite_type
    elif sqlite_type == 'INTEGER':
        return 'TINYINT(1)' if ('is_' in col_name.lower() or 'active' in col_name.lower()) else 'INT'
    elif sqlite_type == 'TEXT':
        return 'LONGTEXT'
    elif sqlite_type in ('REAL', 'FLOAT'):
        return 'DOUBLE'
    elif 'DECIMAL' in sqlite_type:
        return 'DECIMAL(15,2)'
    elif sqlite_type == 'BLOB':
        return 'LONGBLOB'
    elif sqlite_type == 'DATETIME':
        return 'DATETIME'
    elif sqlite_type == 'DATE':
        return 'DATE'
    elif sqlite_type in ('BOOL', 'BOOLEAN'):
        return 'TINYINT(1)'
    return 'LONGTEXT'

def create_mysql_table(mysql_cursor, table_name, schema, pk_cols):
    """Crée une table MySQL"""
    columns = []
    
    for col in schema:
        col_name = col[1]
        col_type = col[2] if col[2] else 'TEXT'
        not_null = col[3]
        default_value = col[4]
        is_pk = col[5]
        
        mysql_type = sqlite_to_mysql_type(col_type, col_name)
        col_def = f"`{col_name}` {mysql_type}"
        
        # AUTO_INCREMENT seulement pour les clés primaires INT
        if is_pk and mysql_type == 'INT':
            col_def += " AUTO_INCREMENT"
        
        if not_null and not is_pk:
            col_def += " NOT NULL"
        
        if default_value is not None and not is_pk:
            if isinstance(default_value, str):
                col_def += f" DEFAULT '{default_value}'"
            else:
                col_def += f" DEFAULT {default_value}"
        
        columns.append(col_def)
    
    if pk_cols:
        pk_def = f"PRIMARY KEY ({', '.join([f'`{col}`' for col in pk_cols])})"
        columns.append(pk_def)
    
    create_query = f"CREATE TABLE `{table_name}` ({', '.join(columns)}) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;"
    
    try:
        mysql_cursor.execute(create_query)
        return True
    except Error as e:
        print(f"  ❌ Erreur création: {str(e)[:150]}")
        return False

def migrate_table_data(sqlite_cursor, mysql_cursor, mysql_conn, table_name):
    """Migre les données d'une table"""
    try:
        # Désactiver les contraintes temporairement
        mysql_cursor.execute("SET FOREIGN_KEY_CHECKS=0;")
        
        sqlite_cursor.execute(f"SELECT * FROM `{table_name}`")
        rows = sqlite_cursor.fetchall()
        
        if not rows:
            print(f"  ℹ️  Table vide")
            mysql_cursor.execute("SET FOREIGN_KEY_CHECKS=1;")
            return True
        
        column_names = [d[0] for d in sqlite_cursor.description]
        placeholders = ', '.join(['%s'] * len(column_names))
        columns_str = ', '.join([f'`{col}`' for col in column_names])
        insert_query = f"INSERT INTO `{table_name}` ({columns_str}) VALUES ({placeholders})"
        
        total = len(rows)
        success = 0
        errors = 0
        
        # Insérer par lots
        batch_size = 100
        for i in range(0, total, batch_size):
            batch = rows[i:i+batch_size]
            for row in batch:
                try:
                    cleaned_row = tuple(val if val is not None else None for val in row)
                    mysql_cursor.execute(insert_query, cleaned_row)
                    success += 1
                except Error as e:
                    errors += 1
                    if errors <= 2:
                        print(f"  ⚠️  Erreur ligne: {str(e)[:80]}")
            
            mysql_conn.commit()
            
            if total > 50:
                print(f"  ⏳ {min(i+batch_size, total)}/{total}...", end='\r')
        
        if total > 50:
            print()  # Nouvelle ligne
        
        print(f"  ✅ {success}/{total} lignes migrées", end='')
        if errors > 0:
            print(f" ({errors} erreurs)")
        else:
            print()
        
        mysql_cursor.execute("SET FOREIGN_KEY_CHECKS=1;")
        return True
        
    except Error as e:
        print(f"  ❌ Erreur migration: {str(e)[:150]}")
        mysql_cursor.execute("SET FOREIGN_KEY_CHECKS=1;")
        return False

def full_migration():
    """Migration complète avec suppression préalable"""
    try:
        print("\n" + "="*70)
        print("  🔄 MIGRATION COMPLÈTE SQLite → MySQL")
        print("  École Hadja Kanfing Dian")
        print("="*70 + "\n")
        
        # Connexion SQLite
        print("📂 Connexion SQLite...")
        sqlite_conn = sqlite3.connect(SQLITE_DB)
        sqlite_cursor = sqlite_conn.cursor()
        print("✅ SQLite connecté\n")
        
        # Connexion MySQL
        print("📊 Connexion MySQL PythonAnywhere...")
        mysql_conn = mysql.connector.connect(**MYSQL_CONFIG)
        mysql_cursor = mysql_conn.cursor()
        print("✅ MySQL connecté\n")
        
        # SUPPRIMER TOUTES LES TABLES
        if not drop_all_tables(mysql_cursor, mysql_conn):
            print("❌ Échec de la suppression des tables")
            return
        
        # Récupérer les tables SQLite
        tables = get_sqlite_tables(sqlite_cursor)
        print(f"📋 {len(tables)} tables à migrer\n")
        print("="*70 + "\n")
        
        success = 0
        errors = 0
        
        # Migrer chaque table
        for idx, table in enumerate(tables, 1):
            print(f"[{idx}/{len(tables)}] 📦 {table}")
            
            schema = get_table_schema(sqlite_cursor, table)
            pk_cols = get_table_pk(sqlite_cursor, table)
            
            if create_mysql_table(mysql_cursor, table, schema, pk_cols):
                mysql_conn.commit()
                if migrate_table_data(sqlite_cursor, mysql_cursor, mysql_conn, table):
                    success += 1
                else:
                    errors += 1
            else:
                errors += 1
            
            print()
        
        print("="*70)
        print(f"✨ MIGRATION TERMINÉE!")
        print(f"✅ Tables réussies: {success}/{len(tables)}")
        if errors > 0:
            print(f"⚠️  Tables avec erreurs: {errors}")
        print("="*70 + "\n")
        
        # Vérification finale
        print("🔍 VÉRIFICATION FINALE:\n")
        
        verif_tables = [
            ('eleves_eleve', '👨‍🎓 Élèves'),
            ('eleves_responsable', '👨‍👩‍👧 Responsables'),
            ('paiements_paiement', '💰 Paiements'),
            ('bus_abonnementbus', '🚌 Abonnements bus'),
            ('eleves_historiqueeleve', '📜 Historique')
        ]
        
        for table, label in verif_tables:
            try:
                mysql_cursor.execute(f"SELECT COUNT(*) FROM `{table}`")
                count = mysql_cursor.fetchone()[0]
                print(f"{label}: {count}")
            except:
                print(f"{label}: Table non trouvée")
        
        print("\n" + "="*70)
        
        sqlite_conn.close()
        mysql_conn.close()
        
    except Error as e:
        print(f"\n❌ Erreur: {e}")
        sys.exit(1)

if __name__ == "__main__":
    print("\n" + "="*70)
    print("  ⚠️  ATTENTION - SUPPRESSION TOTALE")
    print("="*70)
    print("\nCe script va:")
    print("  1. ❌ SUPPRIMER TOUTES les tables MySQL existantes")
    print("  2. ✨ Créer de nouvelles tables propres")
    print("  3. 📦 Migrer TOUTES les données depuis SQLite")
    print("\n" + "="*70)
    
    response = input("\n❓ CONFIRMER la suppression et re-migration? (oui/non): ")
    if response.lower() in ['oui', 'o', 'yes', 'y']:
        full_migration()
        print("\n✅ Migration terminée! Votre base MySQL est maintenant à jour.")
    else:
        print("\n❌ Migration annulée")
