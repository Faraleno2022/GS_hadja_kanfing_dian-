...
def ajouter_annee_scolaire_si_absente(apps, schema_editor):
    """Ajoute la colonne seulement si l'autre branche ne l'a pas déjà créée."""
    ...
    champ.set_attributes_from_name('annee_scolaire')
    champ.model = Paiement
    schema_editor.add_field(Paiement, champ)


def ajouter_unicite_echeancier_si_absente(apps, schema_editor):
    """Préserve l'unicité quel que soit l'historique de migrations installé."""
    Echeancier = apps.get_model('paiements', 'EcheancierPaiement')
    table_name = Echeancier._meta.db_table
    with schema_editor.connection.cursor() as cursor:
        contraintes = schema_editor.connection.introspection.get_constraints(
            cursor,
            table_name,
        )
    nom = 'echeancier_unique_eleve_annee'
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('paiements', '0011_alter_remisereduction_motif'),
    ]
    ...
    operations = [
        migrations.SeparateDatabaseAndState(
            database_operations=[
                migrations.RunPython(
                    ajouter_annee_scolaire_si_absente,
        ...
        migrations.SeparateDatabaseAndState(
            database_operations=[
                migrations.RunPython(
                    ajouter_unicite_echeancier_si_absente,
                    migrations.RunPython.noop,
                ),
            ],
            state_operations=[
