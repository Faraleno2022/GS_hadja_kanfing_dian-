...
class ReportingTempsReelTests(TestCase):
    def setUp(self):
        ...
        Classe.objects.create(
            nom="CM1 rentrée suivante", ecole=self.ecole,
            niveau="PRIMAIRE_4", annee_scolaire="2026-2027",
        )
        ...
        self.eleve = self._creer_eleve(
            "NAB-001", "Camara", "Aminata", self.classe, responsable
        )
        self.eleve_solde = self._creer_eleve(
            "NAB-002", "Diallo", "Mamadou", self.classe, responsable
        )
        self.autre_eleve = self._creer_eleve(
            "AUT-001", "Condé", "Fatou", self.autre_classe, responsable
        )

        self._creer_echeancier(self.eleve, total=100000, paye=20000)
    ...
    def _creer_echeancier(self, eleve, total, paye):
        return EcheancierPaiement.objects.create(
            ...
            # Ce scénario teste une remise de scolarité : le dû et le payé
            # appartiennent donc à T1, jamais aux frais d'inscription.
            frais_inscription_du=0,
            tranche_1_due=total, tranche_2_due=0, tranche_3_due=0,
            frais_inscription_paye=0,
            tranche_1_payee=paye, tranche_2_payee=0, tranche_3_payee=0,
    ...
    @patch(
        'paiements.views_rapport_comptable.timezone.localdate',
    ...
    @patch(
        'rapports.views.django_timezone.localdate',
        return_value=date(2026, 8, 2),
    )
