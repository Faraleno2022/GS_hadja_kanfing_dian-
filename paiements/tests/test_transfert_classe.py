...
class TransfertClassePaiementTests(TestCase):
    ...
    def test_transfert_avec_tarif_inferieur_signale_le_credit(self):
        ...
        self.assertEqual(
            Paiement.objects.filter(eleve=self.eleve, statut="VALIDE").aggregate(total=Sum("montant"))["total"],
            Decimal("600000"),
        )
